...
from loymax import lmx_deposit_page, lmx_login_page, lmx_user_page, lmx_call_center
from bitrix import btx_change_order_page, btx_login_page, btx_order_page, btx_orders_list_page
import pytest
import json
import time
...
class LoyaltyTestBase:
    ...
    def buyout_and_status_change(self, status):
        with allure.step("Обработка заказа в Битрикс"):
            bitrix_login = btx_login_page.BitrixLoginPage(self.driver)
            with allure.step("Авторизуемся в Битрикс"):
                bitrix_login.authorization()
            with allure.step('Переходим в заказ'):
                bitrix_orders_list = btx_orders_list_page.BitrixOrdersListPage(self.driver, self.order)
                bitrix_orders_list.go_to_order_page()
                bitrix_order_page = btx_order_page.BitrixOrderPage(self.driver, self.order)
            with allure.step('Переходим в изменение заказа'):
                bitrix_order_page.go_to_change_order_page()
                bitrix_change_order_page = btx_change_order_page.BitrixChangeOrderPage(self.driver)
            with allure.step('Меняем статус выкупа товара на "Да"'):
                bitrix_change_order_page.change_buyout_status_to_yes()
            with allure.step('Открываем страницу заказа'):
                bitrix_orders_list.go_to_order_page()
            if self.promocode_status:
                with allure.step('Проверяем, что промокод отображается"'):
                    bitrix_order_page.check_promocode_exists()
                with allure.step('Проверяем что сумма заказа совпадает с суммой после применения промокода'):
                    bitrix_order_page.check_order_sum_with_promo()
            if status == "OI":
                time.sleep(60)
            with allure.step(f'Меняем статус заказа на {status}'):
                bitrix_order_page.order_status_change(status)

    def loymax_ops(self, user_with_card):
        with allure.step("Авторизация в loymax"):
            loymax_login = lmx_login_page.LoymaxLoginPage(self.driver)
            with allure.step('Авторизуемся в loymax'):
                loymax_login.authorization()
            if self.bonuses_status:
                with allure.step('Начисляем бонусы'):
                    loymax_deposit = lmx_deposit_page.DepositPage(self.driver, user_with_card)
                    loymax_deposit.deposit_ops()
            with allure.step('Переходим в поиск'):
                call_center_page = lmx_call_center.CallCenterPage(self.driver)
            ...
            with allure.step('Открываем профиль пользователя'):
                loymax_user = lmx_user_page.UserPage(self.driver, self.order)
            with allure.step('Открываем историю пользователя'):
                loymax_user.open_purchase_history()
                loymax_user.get_history_screenshot()
            return loymax_user
            ...
            def handle_bad_result():
                ...
                with allure.step("Списание бонусов недоступно для корзины. Пробуем снова"):
                    ...
                    if submit.check_available_bonuses_is_null():
                        ...
                        login_Page = lmx_login_page.LoymaxLoginPage(self.driver)
                        with allure.step('Авторизуемся в loymax'):
